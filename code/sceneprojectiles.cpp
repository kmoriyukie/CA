...
SceneProjectiles::SceneProjectiles() {
    ...
    const std::vector<std::string> solvers = {
        "Euler", "Symplectic Euler", "Midpoint","Verlet"
    };
...
void SceneProjectiles::reset() {
    ...
    systemAnalytic.getParticle(0)->prevPos = systemAnalytic.getParticle(0)->pos;
    ...
    systemNumerical1.getParticle(0)->prevPos = systemNumerical1.getParticle(0)->pos;
    ...
    systemNumerical2.getParticle(0)->prevPos = systemNumerical2.getParticle(0)->pos;
...
void SceneProjectiles::update(double dt) {
    ...
    ColliderPlane *plane = new ColliderPlane();
    plane->setPlane(Vec3(0, 1, 0), 0);

    if (system1active) {
        // integration step
        Vecd ppos = systemNumerical1.getPositions();
        integrator1->step(systemNumerical1, dt);
        systemNumerical1.setPreviousPositions(ppos);

        // collision test
        Particle* p = systemNumerical1.getParticle(0);
        if (plane->testCollision(p)) {
            plane->resolveCollision(p, 0.6, 0.1);
        }
    ...
    if (system2active) {
        // integration step
        Vecd ppos = systemNumerical2.getPositions();
        integrator2->step(systemNumerical2, dt);
        systemNumerical2.setPreviousPositions(ppos);
